...
const { checkPassword, hashPassword, getObjectId } = require("../../utils/helper")
...
exports.login = async (reqParams) => {
 try {
  ...
  if (checkPwdStatus == false) {
   return { "status": AUTH_ERROR_CODE, "msg": "Invalid password" }
  }
  delete userData[0]["password"]
  const tokenRes = await generateToken(userData[0])
  if (tokenRes.status) {
   return { "msg": "Login successful", "token": tokenRes["token"] };
  } else {
